...
class AmountRankingService(RankingBaseService):
    async def get_domestic(self):
        ...
        if data and 'output' in data:
            # 원본 데이터 정렬 (거래대금 내림차순)
            data['output'].sort(key=lambda x: int(x.get('acml_tr_pbmn', 0)), reverse=True)
            ...
            for item in data['output']:
                normalized.append({
                    ...
                    "amount": float(item.get('acml_tr_pbmn', 0)),  # 거래대금
                    "value": float(item.get('acml_tr_pbmn', 0)),   # 정렬 기준 (거래대금)
                    "market": "domestic"
                })
    ...
    async def get_overseas(self, nday="0", excd="NAS"):
        ...
        for item in data.get('output2', []):
            ...
            normalized.append({
                ...
                "amount": amount_krw,                   # 거래대금 (원화 환산)
                "value": amount_krw,                    # 정렬 기준 (거래대금)
                "market": "overseas"
            })
